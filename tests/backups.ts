...
import { AddressParams, SetSpawnProxy } from "../client/typescript/build";
...
const planetOwnershipAddress: EthAddress =
  "0x6d654ef2489674d21aed428e8a4ad8ca4820f125";
const planetInviteAddress: EthAddress =
  "0x6d654ef2489674d21aed428e8a4ad8ca4820f125";
...
//   // const uno = await api.getNaiveState();
//   // expect(uno).toHaveProperty("points");
//   // const dos = await api.getPredictedState();
//   // expect(dos).toHaveProperty("points");
//   const tres = await api.getPendingTx("0x1234");
//   console.log(tres);
...
test("getPoint", async () => {
  ...
  // api.onError((err: any) => console.log(err.message));

  try {
    const point: L2Point | JSONRPCError = await api.getPoint(ship);
    console.log(point);
    expect(point).toHaveProperty("dominion", "l2");
  } catch (point) {
...
test("~wanzod sets its spawn proxy", async () => {
  const spawnProxyData: AddressParams = {
    address: starAddress,
  };
  const wanzod: L2Point = await api.getPoint("~wanzod");
  const nonce: Nonce = wanzod.ownership?.owner?.nonce!;
  ...
  const proxyTxHash: Hash = await api.setSpawnProxy(
  ...
  const txStatus: TransactionStatus = await api.getTransactionStatus(
    proxyTxHash
  );
  expect(txStatus).toEqual("pending");
});
...
//   const transferTxHash: Hash = await api.transferPoint(
//     signMessage(transferHash, planetReceivingAddress),
//     fromBalhulTransfer,
//     planetReceivingAddress,
//     transferData
...
//
//   The sponsor star configures keys and sets the planet's management proxy
//   The invitee sets networking keys before transfering to a new address
//   And sets a new management proxy logging in with the new Ownership key (master ticket)
//
// test("~wanzod sets its spawn proxy", async () => {
//   const spawnProxyData: AddressParams = {
//     address: starAddress,
...
// // The following three tests correspond to the "Send Invite" flow
...
// test("...sets its management proxy to the same as the transfer address", async () => {
//   const mgmtData = { address: planetReceivingAddress };
//   const mgmtHash: Hash = await api.hashTransaction(
//     0,
//     fromModlepOwn,
//     "setManagementProxy",
...
// test("and configures its keys so it's ready to boot", async () => {
//   const configureKeysData = {
//     encrypt: "0x1234",
//     auth: "0xabcd",
//     cryptoSuite: "0",
...
// // The following three tests correspond to the "Accept Invite" flow

// test("~modlep-fosreg accepts the transfer by configuring new networking keys", async () => {
//   const configureKeysData = {
//     encrypt: "0x1234",
//     auth: "0xabcd",
//     cryptoSuite: "0",
...
//   const configureTxHash = await api.configureKeys(
//     signMessage(configureKeysHash, planetReceivingAddress),
//     fromModlepManage,
//     planetReceivingAddress,
//     configureKeysData
//   );
...
// test("and transfering to a new address", async () => {
//   //
//   //    ("sig" is signed using nonce 0)
//   //
//   const transferData = { address: planetOwnershipAddress, reset: false };
//   // const modlep: L2Point = await api.getPoint("~modlep-fosreg");
//   const transferHash: Hash = await api.hashTransaction(
//     0,
//     // ~modlep-fosreg is not spawned yet, since the txs are all pending
...
// test("and finally, setting up a new management proxy, logging in with the new ownership address", async () => {
//   const mgmtData = { address: planetMgmtAddress };
//   const mgmtHash: Hash = await api.hashTransaction(
//     2,
//     fromModlepOwn,
...
// L2 Invite Flow - D -
//
//   The sponsor star, after spawning a planet, transfers it to a new address
//   (obtained, for example by generating a new wallet, similar to what Bridge does)
//   Then configures the planet's networking keys so it's ready to boot
//
//   The invitee logs in with a planet+ticket pair, creates a new ownership address
//   (for example generating a new wallet, similat to Bridge),
//   sets networking keys (using the owner proxy)
//   and then transfers the point (also with owner proxy)
//   to the address associated with the new wallet
//
test("~wanzod sets its spawn proxy", async () => {
  const spawnProxyData: AddressParams = {
    address: starAddress,
...
test("... transfers it to the new address", async () => {
  const transferData = { address: planetInviteAddress, reset: true };
  const transferHash: Hash = await api.hashTransaction(
    0,
    fromModlepOwn,
    "transferPoint",
  ...
  const transferTxHash: Hash = await api.transferPoint(
    signMessage(transferHash, starAddress),
    fromModlepOwn,
    starAddress,
    transferData
  );
...
test("and configures its keys so it's ready to boot", async () => {
  ...
  const configureTxHash = await api.configureKeys(
    signMessage(configureKeysHash, planetInviteAddress),
    fromModlepOwn,
    planetInviteAddress,
    configureKeysData
  );
...
// The following two tests correspond to the "Accept Invite" flow
...
test("and transfer to a new address", async () => {
  //
  //    ("sig" is signed using nonce 0)
  //
  const transferData = { address: planetOwnershipAddress, reset: false };
  ...
  const transferTxHash = await api.transferPoint(
    signMessage(transferHash, planetInviteAddress),
    fromModlepOwn,
    planetReceivingAddress,
    transferData
  );
